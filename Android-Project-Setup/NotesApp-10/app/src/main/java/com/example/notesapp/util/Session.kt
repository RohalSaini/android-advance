...
class Session(var ctx: Context) {
    ...
    fun setDataFromSharedPreferences(list: MutableList<com.example.notesapp.modal.TodoData>) {
        val gson = Gson()
        val jsonCurProduct: String = gson.toJson(list)
        editor.putString("list", jsonCurProduct)
        editor.commit()
    }
    fun getDataFromSharedPreferences(): MutableList<com.example.notesapp.modal.TodoData> {
            val gson = Gson()
            val list:MutableList<com.example.notesapp.modal.TodoData>
            val string: String? = prefs.getString("list","empty")
            val type: Type = object : TypeToken<MutableList<com.example.notesapp.modal.TodoData?>?>() {}.type
            list = gson.fromJson<MutableList<com.example.notesapp.modal.TodoData>>(string, type)
            return list
    }
}